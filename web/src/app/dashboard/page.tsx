...
export default function Dashboard() {
  ...
  const [apiStats, setApiStats] = useState({
    ide_gemini: { prompt: 0, completion: 0 },
    ide_claude: { prompt: 0, completion: 0 },
    terminal_claude: { prompt: 0, completion: 0 },
  });
  ...
  useEffect(() => {
    ...
    const unsubApi = onSnapshot(apiQ, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        let targetKey = `${source}_${provider}`;
        // Fallback for existing data
        if (!stats[targetKey as keyof typeof stats]) {
          if (provider === 'gemini') targetKey = 'ide_gemini';
          else if (provider === 'claude') targetKey = 'terminal_claude';
        }

        if (stats[targetKey as keyof typeof stats]) {
          stats[targetKey as keyof typeof stats].prompt += pTokens;
          stats[targetKey as keyof typeof stats].completion += cTokens;
        }
      });
      ...
      <section className="max-w-4xl mx-auto mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-blue-900/40 to-black p-6 rounded-2xl border border-blue-500/20 relative overflow-hidden group hover:border-blue-500/50 transition-colors">
            <h3 className="text-lg font-medium text-blue-400 mb-2">Antigravity (Gemini)</h3>
            <div className="text-3xl font-bold text-white mb-2">
              {((apiStats.ide_gemini.prompt + apiStats.ide_gemini.completion) / 1000).toFixed(1)}k <span className="text-sm text-gray-400 font-normal">Tokens</span>
            </div>
            <div className="space-y-2 text-xs text-gray-300 mt-6">
              <div className="flex justify-between">
                <span>入力:</span>
                <span className="font-mono">{apiStats.ide_gemini.prompt.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span>出力:</span>
                <span className="font-mono">{apiStats.ide_gemini.completion.toLocaleString()}</span>
              </div>
            </div>
          </div>

          {/* Antigravity (Claude) Card */}
          <div className="bg-gradient-to-br from-purple-900/40 to-black p-6 rounded-2xl border border-purple-500/20 relative overflow-hidden group hover:border-purple-500/50 transition-colors">
            <h3 className="text-lg font-medium text-purple-400 mb-2">Antigravity (Claude)</h3>
            <div className="text-3xl font-bold text-white mb-2">
              {((apiStats.ide_claude.prompt + apiStats.ide_claude.completion) / 1000).toFixed(1)}k <span className="text-sm text-gray-400 font-normal">Tokens</span>
            </div>
            <div className="space-y-2 text-xs text-gray-300 mt-6">
              <div className="flex justify-between">
                <span>入力:</span>
                <span className="font-mono">{apiStats.ide_claude.prompt.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span>出力:</span>
                <span className="font-mono">{apiStats.ide_claude.completion.toLocaleString()}</span>
              </div>
            </div>
          </div>

          {/* Terminal (Claude) Card */}
          <div className="bg-gradient-to-br from-orange-900/40 to-black p-6 rounded-2xl border border-orange-500/20 relative overflow-hidden group hover:border-orange-500/50 transition-colors">
            <h3 className="text-lg font-medium text-orange-400 mb-2">Terminal (Claude)</h3>
            <div className="text-3xl font-bold text-white mb-2">
              {((apiStats.terminal_claude.prompt + apiStats.terminal_claude.completion) / 1000).toFixed(1)}k <span className="text-sm text-gray-400 font-normal">Tokens</span>
            </div>
            <div className="space-y-2 text-xs text-gray-300 mt-6">
              <div className="flex justify-between">
                <span>入力:</span>
                <span className="font-mono">{apiStats.terminal_claude.prompt.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span>出力:</span>
                <span className="font-mono">{apiStats.terminal_claude.completion.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
